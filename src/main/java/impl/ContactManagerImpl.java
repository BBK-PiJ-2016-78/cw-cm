...
public class ContactManagerImpl implements ContactManager, Serializable {
  ...
  public int addFutureMeeting(Set<Contact> contacts, Calendar date)
    ...
    if (date != null && futureMeetingList != null) {
      if (!date.before(Calendar.getInstance()) && !contacts.contains(null)) {
        ...
        if (size == 0) {
          id++;
          //IDs for futureMeetings are odd
        } else if (futureMeetingList.get(size - 1).getId() % 2 == 0) {
  ...
  public List<Meeting> getFutureMeetingList(Contact contact)
    ...
    for (int i = 0; i < streamList.size() - 1; i++) {
      //remove duplicates with same date
  ...
  public List<Meeting> getMeetingListOn(Calendar date)
    ...
    for (int i = 0; i < streamList.size() - 1; i++) {
      if (streamList.get(i).getDate().compareTo(streamList.get(i + 1).getDate()) == 0
          ...
          //remove duplicates with same date and contacts
          && streamList.get(i + 1).getContacts().containsAll(contactSet)) {
        streamList.remove(i);
      }
    }
   ...
   * @throws IllegalArgumentException if contacts is empty, contact doesn't exist or date in future.
  ...
  public int addNewPastMeeting(Set<Contact> contacts, Calendar date, String text)
    ...
    if (date != null && contacts != null && text != null) {
      if (date.before(Calendar.getInstance()) && !contacts.contains(null) && contacts.size() != 0) {
        ...
        if (size == 0) {
          id = 2;
          //IDs for pastMeetings are even
        } else if (pastMeetingList.get(size - 1).getId() % 2 != 0) {
  ...
  public PastMeeting addMeetingNotes(int id, String text)
    ...
    //If any of the future meetings already occurred add it to past list
    for (Meeting count : futureMeetingList) {
      if (count.getDate().before(Calendar.getInstance())) {
        pastMeetingList.add(count);
      }
    }
  ...
  /**
   * To be used to load all data from files, when first launched and before writing new data.
   */
  public void loadData() {
    ...
    //Load futureMeetings data from file
    try (FileInputStream fis = new FileInputStream(futureMeetingsLoad)) {
    ...
    //Load pastMeetings data from file
    try (FileInputStream fis = new FileInputStream(pastMeetingsLoad)) {
    ...
    //check if some of the dates in futureMeetingList are expired and remove them from list
    ...
    for (Meeting count : futureMeetingList) {
      if (count.getDate().compareTo(Calendar.getInstance()) == -1) {
        ...
        pastMeetingList.add(
            new PastMeetingImpl(newId, count.getDate(), count.getContacts(), "past meeting"));
  ...
  public void flush() {
    ...
    //Write futureMeetings data to file
    ...
    //Write pastMeetings data to file
